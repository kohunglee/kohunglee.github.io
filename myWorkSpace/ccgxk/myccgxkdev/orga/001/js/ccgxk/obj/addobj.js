...
export default {
    ...
    initBodyTypeArray : function(MAX_BODIES = 1_000_000){  // 根据最多物体数量，初始化
        ...
        this.physicsProps = new Float32Array(MAX_BODIES * 8);  // [mass, width, height, depth, /*DPZ*/]
    ...
    addTABox : function({
        ...
        // this.physicsProps[p_offset + 4] = DPZ;  // DPZ（显示优先级  1->永远显示 2->1000m显示 3->100m显示 4->20m显示）
        const dpzGirdSize = this.gridsize[DPZ];
        const gridKey = `${dpzGirdSize}_${Math.floor(X / dpzGirdSize)}_${Math.floor(Z / dpzGirdSize)}`;  //+5 计算区块 key，并填进数组，再填入表
