...
export default {
    ...
    gridsize : new Uint16Array([10000, 1000, 100, 20]),  // 单个区块面积大小，根据 DPZ 来判断，（显示优先级  1->永远显示 2->1000m显示 3->100m显示 4->20m显示）
    ...
    dynaNodes_lab : function(){
        if(this.mainVPlayer === null || this.stopDynaNodes) {return ''};
        var mVP = this.mainVPlayer;
        const playerGridX = Math.floor(mVP.X / this.gridsize[4]);  //+8 计算主角周围 9 个格子的区块
        const playerGridZ = Math.floor(mVP.Z / this.gridsize[4]);
        ...
        for(const key of activeGridKeys){
            ...
            if (indicesInGrid) {
                for (const index of indicesInGrid) {
                    if(Math.abs(this.positionsStatus[index * 8 + 1] - mVP.Y) < this.gridsize[4]){  // 高度距离（Y）要接近
